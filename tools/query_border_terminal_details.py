"""Agent tool to query border terminal details by zone."""
...
@tool
def query_border_terminal_details(zone: str) -> str:
    """Border terminals: lists each terminal with status plus a total row per status showing count in the given zone. zone: e.g. 'B'."""
    w, wh_id = get_warehouse()
    sql = substitute_schema((_FUNC_DIR / "border_terminal_details.sql").read_text().strip())
