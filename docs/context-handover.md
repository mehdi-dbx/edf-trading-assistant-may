...
| **app/** | Cloned template: client (Vite/React), server (Express), packages (auth, ai-sdk-providers, chat-template/core) |
...
- **app/server/src/routes/tables.ts** – `ALLOWED_TABLES`, SQL per table; add new tables and allowlist
- **app/client/src/lib/response-blocks.ts** – Parses agent text into blocks; add new block types and regex
- **app/client/src/components/message.tsx** – Renders blocks (RefreshTableTrigger, FollowUpActions, Cards)
...
- `app/client/src/contexts/TableRefreshContext.tsx`
- `app/client/src/contexts/RoleContext.tsx` (Manager/Agent dropdown)
- `app/client/src/contexts/ChatSendMessageContext.tsx` (registerSendMessage, sendCheckTasks)
- `app/client/src/hooks/useTableData.ts`
- `app/client/src/components/message.tsx` (RefreshTableTrigger)
- `app/server/src/routes/tables.ts`
