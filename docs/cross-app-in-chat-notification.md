...
|--------------|--------------|
| **Same codebase structure** | Both projects use the app layout: Node API (Express), React frontend, `useChat` from Vercel AI SDK, `Chat` component with `setMessages` |
...
| Purpose | Path |
|---------|------|
| Node API, task-events pattern | `app/server/src/index.ts` (lines 137–192) |
| SSE client pattern | `app/client/src/hooks/useTaskEvents.ts` |
| Append message pattern | `app/client/src/components/chat.tsx` (lines 291–306, `showStaffingDutyOnly`) |
| Chat component, setMessages | `app/client/src/components/chat.tsx` |
| generateUUID | `app/client/src/lib/utils.ts` or `@chat-template/core` |
