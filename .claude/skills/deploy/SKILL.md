...
The bundle resource key is `agent_edf_trading_assistant` under `resources.apps`; the Databricks App **name** is `DBX_APP_NAME` (synced into `databricks.yml` targets).
...
databricks bundle deployment bind agent_edf_trading_assistant <existing-app-name> --auto-approve
```
...
```bash
databricks bundle deployment unbind agent_edf_trading_assistant
```
...
| App running old code after deploy | Run `databricks bundle run agent_edf_trading_assistant` after deploy |
