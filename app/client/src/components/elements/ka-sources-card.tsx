...
function volumePathToUrl(path: string, excerpt?: string): { displayName: string; url: string } | null {
  if (!path.startsWith('/Volumes/')) return null;
  const basename = path.split('/').pop() ?? path;
  const base = `/api/files?path=${encodeURIComponent(path)}`;
  const fragment = excerpt
    ? `#search=${encodeURIComponent(excerpt.trim().split(/\s+/).slice(0, 7).join(' '))}`
    : '';
  return { displayName: basename, url: `${base}${fragment}` };
}
...
function normalizeSourceRow(raw: Record<string, unknown>): KaSourceItem | null {
  ...
  const volumeLink = volumePathToUrl(rawTitle, excerpt);
  if (volumeLink) {
