...
function getAuthMethodDescription() {
	...
	switch (method) {
		...
		case "pat": return "PAT (Personal Access Token)";
...
async function getDatabricksToken() {
	...
	switch (method) {
		...
		case "pat": {
			...
			if (!token) throw new Error("DATABRICKS_TOKEN is not set");
...
async function getDatabaseUsername() {
	...
	switch (method) {
		...
		case "pat": return process.env.PAT_USER || process.env.USER || process.env.USERNAME || "pat-user";
		case "cli":
