...
const ALLOWED_TABLES = ["example_data"];
...
filesRouter.get("/", async (req, res) => {
	const path$1 = typeof req.query.path === "string" ? req.query.path : "";
	if (!path$1.startsWith("/Volumes/")) return res.status(400).json({ error: "path must start with /Volumes/" });
	let base = (process.env.API_PROXY || "").replace(/\/invocations\/?$/, "") || "http://127.0.0.1:8000";
	if (base.startsWith("http://localhost:") || base.startsWith("https://localhost:")) base = base.replace("localhost", "127.0.0.1");
	const url = `${base}/files?path=${encodeURIComponent(path$1)}`;
	try {
